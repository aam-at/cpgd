...
from attack import DeepFool, FastGradientMethod, HighConfidenceAttack
from data import load_mnist, make_input_pipeline
from models import create_model, register_model_flags
from utils import (MetricsDictionary, compute_norms, load_experiment,
                   log_metrics, register_experiment_flags, save_images,
                   setup_experiment)
...
# FGSM
flags.DEFINE_float("at_epsilon1", 0.1, "fast gradient epsilon")
flags.DEFINE_float("at_epsilon2", 0.3, "fast gradient epsilon")
...
def main(unused_args):
    ...
    # attacks
    deepfool = DeepFool(lambda x: test_classifier(x)["logits"],
    ...
    hc = HighConfidenceAttack(
        ...
        boxmin=0.0,
        boxmax=1.0)
    at1 = FastGradientMethod(lambda x: test_classifier(x)["logits"],
                             epsilon=FLAGS.at_epsilon1,
                             boxmin=0.0, boxmax=1.0)
    at2 = FastGradientMethod(lambda x: test_classifier(x)["logits"],
    ...
    def test_step(image, label):
        ...
        test_metrics["ssim_df"](tf.image.ssim(
            tf.transpose(image, (0, 2, 3, 1)),
            tf.transpose(image_df, (0, 2, 3, 1)),
            max_val=1.0))
    ...
    for batch_index, (image, label) in enumerate(test_ds, 1):
        ...
        save_images(X_df.numpy(), save_path)
        save_path = os.path.join(FLAGS.samples_dir, 'epoch_hc-%d.png' % batch_index)
        save_images(X_hc.numpy(), save_path)
        save_path = os.path.join(FLAGS.samples_dir, 'epoch_at1-%d.png' % batch_index)
        save_images(X_at1.numpy(), save_path)
        save_path = os.path.join(FLAGS.samples_dir, 'epoch_at2-%d.png' % batch_index)
        save_images(X_at2.numpy(), save_path)
    log_metrics(
