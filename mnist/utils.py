...
def setup_experiment(default_name, script_name=None):
    ...
    if script_name is not None:
        copyfile(script_name, os.path.join(FLAGS.working_dir, script_name))


def load_experiment(path,
                    attrs=[
                        'model', 'model_name', 'activation',
                        'feature_layer_dims', 'classifier_layer_dims',
                        'layer_type'], prefix=''):
    load_from = Path(path)
    if not load_from.exists() or not load_from.is_dir():
        raise ValueError("Could not find folder %s" % FLAGS.load_dir)
    train_params = load_training_params(load_from)
    for attr in attrs:
        setattr(FLAGS, prefix + attr, train_params[attr])
...
def kl(q, p, epsilon=1e-4):
    with tf.name_scope("kl_divergence") as name:
        return tf.reduce_sum(
            q * (tf.math.log(q + epsilon) - tf.math.log(p + epsilon)),
            axis=1,
            name=name)
...
def kl_with_logits(q_logits, p_logits):
    with tf.name_scope("kl_divergence") as name:
        ...
        return tf.reduce_sum(q * (q_log - p_log), axis=1, name=name)


def kl_binary(q, p, epsilon=1e-12):
    delta_probs0 = tf.math.log(q + epsilon) - tf.math.log(p + epsilon)
    delta_probs1 = tf.math.log1p(-q + epsilon) - tf.math.log1p(-p + epsilon)
    return q * delta_probs0 + (1 - q) * delta_probs1


def kl_binary_with_logits(q_logits, p_logits):
    delta_probs0 = tf.nn.softplus(-p_logits) - tf.nn.softplus(-q_logits)
    delta_probs1 = tf.nn.softplus(p_logits) - tf.nn.softplus(q_logits)
    return tf.nn.sigmoid(q_logits) * delta_probs0 + tf.nn.sigmoid(-q_logits) * delta_probs1
...
def gaussian_noise(x, stddev, scope=None):
    if stddev != 0.0:
        with tf.variable_scope(scope, 'wnoise'):
            return x + tf.random_normal(
                shape=tf.shape(x), mean=0., stddev=stddev)
    else:
        return x
