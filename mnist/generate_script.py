...
from lib.parse_logs import parse_test_optimizer_l2_log
...
def test_l2_config(runs=1, master_seed=1):
    for (model, max_iter, opt, tol,
         sampling_radius) in itertools.product(models, [1000],
                                               ["adam"], [0.005],
                                               [2, 4, 6, 8, 10, 12]):
        type = Path(model).stem.split("_")[-1]
        ...
        finetune = True
        finetunestr = "finetune" if finetune else "nofinetune"
        for lr, llr, r0_init, C0 in itertools.product([0.1, 0.05, 0.01],
                                                      [0.1, 0.05],
                                                      ["sign", "uniform"],
                                                      [0.1, 0.5]):
            np.random.seed(master_seed)
...
def test_l2_config_custom(runs=1, master_seed=1):
    df = parse_test_optimizer_l2_log("../results/mnist_final/test_l2_plain",
                                     export_test_params=["attack_%s" % n for n in ["optimizer", "finetune", "primal_lr", "dual_lr", "r0_init", "sampling_radius", "initial_const", "tol", "max_iter"]])
